...
namespace Senegal\Api\SdkBundle\Tests\Security\User;

use Pfd\Sdk\Bridge\LegacyBridge;
use Pfd\Sdk\Model\User;
use Senegal\Api\SdkBundle\Security\User\SdkUserProvider;
...
class SdkUserProviderTest extends \PHPUnit_Framework_TestCase
{
    ...
    {
        $this->sdk = $this->getMockBuilder('Pfd\Sdk\User\UserSdk')
            ->disableOriginalConstructor()
            ->getMock();

        $this->user = $this->getMockBuilder('Pfd\Sdk\Model\User')
            ->disableOriginalConstructor()
            ->getMock();

        $this->bridge = $this->getMockBuilder('Pfd\Sdk\Bridge\LegacyBridge')
            ->disableOriginalConstructor()
            ->getMock();
     ...
     * @expectedExceptionMessage Instances of "Senegal\Api\SdkBundle\Tests\Security\User\SomeUser" are not supported.
     */
    ...
    public function testSupportsClass()
    {
        ...
        $this->assertTrue($this->provider->supportsClass('Pfd\Sdk\Model\User'));
    }
}
