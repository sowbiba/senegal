...
namespace Senegal\Api\SdkBundle\DependencyInjection\CompilerPass;
...
class SdkEventListenersPass implements CompilerPassInterface
{
    public function process(ContainerBuilder $container)
    {
        ...
        $listeners  = $container->findTaggedServiceIds('pfd_sdk.event_listener');
        ...
        foreach ($listeners as $id => $instances) {
            foreach ($instances as $instance) {
                ...
                $definition->addMethodCall(
                    'addListener',
                    array(sprintf("pfd_sdk_%s", $instance['event']), array(new Reference($id), $instance['event']))
                );
            }
        }
    }
